let tempoUltimoTiroNivel3 = 0; // Inicializa o tempo do último disparo no nível 3
let delayDisparoNivel3 = 200; // Define um intervalo de 200ms entre os disparos no nível 3

let nivelCompletoNivel3 = false; // Flag para verificar se o nível foi completado
let velocidadeZumbisNivel3 = 2.5; // Define a velocidade dos zumbis no nível 3

function iniciarNivel3() {
  // Configurações iniciais para o nível 3
  tela = 2; // Define a tela de jogo
  nivel = 3; // Define que o nível actual é o 3
  vidas = 3; // Reinicializa o número de vidas do jogador
...
function desenharNivel3() {
  background(150); // Define o fundo padrão para o nível 3

  // Verifica se a pontuação necessária para completar o nível foi atingida
  if (pontos >= 30) {
    nivelCompletoNivel3 = true; // Define o nível como completo
    fill(255); // Cor do texto
    textSize(32); // Tamanho do texto
    textAlign(CENTER, CENTER); // Alinhamento do texto
    text('Nível 3 Completo! Clique para nível 4', width / 2, height / 2); // Mensagem de conclusão do nível
    
    // Verifica se o botão do rato foi pressionado para iniciar o nível 4
    if (mouseIsPressed) {
      iniciarNivel4(); // Chama a função para iniciar o próximo nível
    }
  ...
  // Controle de disparo com base no volume do microfone e no delay de disparo
  let volume = mic.getLevel(); // Obtém o nível de volume do microfone
  if (volume > 0.1 && millis() - tempoUltimoTiroNivel3 > delayDisparoNivel3) {
    heroi.atirar(projeteis, nivel); // O herói dispara projéteis se o volume for suficiente
    tiroSom.play(); // Toca o som do disparo
    tempoUltimoTiroNivel3 = millis(); // Actualiza o tempo do último disparo
  }
  ...
  fill(255);
  textSize(16); // Tamanho do texto para as informações do aluno
  ...
  if (alunos.length === 0 && pontos < 30) {
    tela = 3; // Muda para a tela de fim de jogo
  }
}

// Função para transição de nível após completar o nível actual
function mousePressed() {
  if (nivelCompletoNivel3) {
    iniciarNivel4(); // Inicia o nível 4 ao completar o nível 3
  }
}
